...
const UserData = ({ user, onEdit, onDelete }) => {
	const { ID, user_email, first_name, last_name } = user;
		...
		<>
			{usersList.length > 0 && (
				<div className={'userslist_wrapper'}>
					<div className={'temp__user_data temp__user_data_headings'}>
						{map(Headings, (heading) => (
							<div
								key={heading}
								className={
									'temp_user_data_item temp_user_item_heading'
								}
							>
								{heading}
							</div>
					...
					{map(usersList, (user) => {
						const { ID } = user;
						return <UserData key={ID} user={user} {...props} />;
					})}
				</div>
			)}

			{!usersList.length && (
				<p className={'tempaccess__no_users'}>
					{__('No Users Found', 'temporary-access')}
				</p>
			)}
		</>
	);
